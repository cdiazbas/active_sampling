...
def f_nn(x,edges=None,ni_epochs=5000):
    ...
    plt.yscale('log')
    plt.minorticks_on()
    plt.savefig('sampling_uniform/error_'+str(inpoint)+'.pdf')
    plt.close(fig1)
    ...
    plt.savefig('sampling_uniform/stokes_error_temp'+str(input_size)+'.png')
    plt.close(fig1)

    np.save('sampling_uniform/stokes_error_temp'+str(input_size)+'.npy',diff.detach().numpy())
...
for inpoint in range(3,npoints+1):
    ...
    plt.savefig('sampling_uniform/stokes_reconstruction_temp_'+str(len(x))+'.png')


# Save final sampling
np.save('sampling_uniform/final_sampling.npy',x)
